...
* under the License.
*/
import { each, map, isFunction, createHashMap, noop, assert } from 'zrender/src/core/util';
...
class Scheduler {
    ...
    _createSeriesStageTask(stageHandler, stageHandlerRecord, ecModel, api) {
        ...
        function create(seriesModel) {
            ...
            const task = newSeriesTaskMap.set(pipelineId, oldSeriesTaskMap && oldSeriesTaskMap.get(pipelineId)
                || createTask({
                    plan: seriesTaskPlan,
    ...
    _createOverallStageTask(stageHandler, stageHandlerRecord, ecModel, api) {
        const scheduler = this;
        const overallTask = stageHandlerRecord.overallTask = stageHandlerRecord.overallTask
        ...
        function createStub(seriesModel) {
            const pipelineId = seriesModel.uid;
            const stub = newAgentStubMap.set(pipelineId, oldAgentStubMap && oldAgentStubMap.get(pipelineId)
                || (
