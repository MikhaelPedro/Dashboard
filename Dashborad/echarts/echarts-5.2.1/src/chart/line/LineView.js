...
function clipColorStops(colorStops, maxSize) {
    ...
    function lerpStop(stop0, stop1, clippedCoord) {
        ...
        const color = lerp(p, [stop0.color, stop1.color]);
        return { coord: clippedCoord, color };
    }
    for (let i = 0; i < len; i++) {
...
function getVisualGradient(data, coordSys, api) {
    ...
    const colorStops = zrUtil.map(visualMeta.stops, function (stop) {
...
function getIsIgnoreFunc(seriesModel, data, coordSys) {
    ...
    zrUtil.each(categoryAxis.getViewLabels(), function (labelItem) {
        const ordinalNumber = categoryAxis.scale
...
class LineView extends ChartView {
    ...
    _newPolygon(points, stackedOnPoints) {
        let polygon = this._polygon;
    ...
    _doUpdateAnimation(data, stackedOnPoints, coordSys, api, step, valueOrigin) {
        const polyline = this._polyline;
        const polygon = this._polygon;
        const seriesModel = data.hostModel;
