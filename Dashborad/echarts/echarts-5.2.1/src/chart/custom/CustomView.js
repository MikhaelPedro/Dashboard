...
* under the License.
*/
import { hasOwn, assert, isString, retrieve2, retrieve3, defaults, each, keys, bind, eqNaN, indexOf } from 'zrender/src/core/util';
...
export default class CustomChartView extends ChartView {
    constructor() {
        super(...arguments);
        this.type = CustomChartView.type;
    }
...
api, el, dataIndex, elOption, attachedTxInfo, seriesModel, isInit, isTextContent) {
    ...
    if (styleOpt) {
        ...
        let decalPattern;
        const decalObj = isPath(el) ? styleOpt.decal : null;
        if (api && decalObj) {
            decalObj.dirty = true;
...
function applyPropsTransition(el, dataIndex, seriesModel, 
// Can be null/undefined
...
function makeRenderItem(customSeries, data, ecModel, api) {
    ...
    function visual(visualType, dataIndexInside) {
        ...
        if (hasOwn(STYLE_VISUAL_TYPE, visualType)) {
            const style = data.getItemVisual(dataIndexInside, 'style');
            return style
                ? style[STYLE_VISUAL_TYPE[visualType]]
                : null;
        }
...
function processTxInfo(elOption, state, attachedTxInfo) {
    ...
    if (!state && txConOpt) {
        ...
        if (__DEV__) {
            ...
            assert(txConOptNormal.type === 'text', 'textContent.type must be "text"');
        }
    }
