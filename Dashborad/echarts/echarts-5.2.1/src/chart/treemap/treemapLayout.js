...
export default {
    ...
    reset: function (seriesModel, ecModel, api, payload) {
        ...
        const layoutInfo = layout.getLayoutRect(seriesModel.getBoxLayoutParams(), {
            width: api.getWidth(),
            height: api.getHeight()
        });
        const size = seriesOption.size || []; // Compatible with ec2.
        const containerWidth = parsePercent(retrieveValue(layoutInfo.width, size[0]), ecWidth);
        const containerHeight = parsePercent(retrieveValue(layoutInfo.height, size[1]), ecHeight);
        // Fetch payload info.
        ...
        const viewAbovePath = helper.getPathToRoot(viewRoot);
...
function worst(row, rowFixedLength, ratio) {
    ...
    return squareArea
        ? mathMax((f * areaMax) / squareArea, squareArea / (f * areaMin))
        : Infinity;
}
/**
