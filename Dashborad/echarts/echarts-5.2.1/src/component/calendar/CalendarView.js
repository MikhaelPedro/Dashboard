...
class CalendarView extends ComponentView {
    ...
    _getEdgesPoints(points, lineWidth, orient) {
    ...
    // render split line
    _drawSplitline(points, lineStyle, group) {
    ...
    _yearTextPositionControl(textEl, point, orient, position, margin) {
        let x = point[0];
        let y = point[1];
    ...
    _monthTextPositionControl(point, isCenter, orient, position, margin) {
        let align = 'left';
        let vAlign = 'top';
        let x = point[0];
        let y = point[1];
    ...
    _renderMonthText(calendarModel, orient, group) {
        ...
        for (let i = 0; i < termPoints[idx].length - 1; i++) {
            ...
            const monthText = new graphic.Text({
                z2: 30,
                style: zrUtil.extend(createTextStyle(monthLabel, { text: content }), this._monthTextPositionControl(tmp, isCenter, orient, pos, margin))
            });
            group.add(monthText);
        }
    }
    _weekTextPositionControl(point, orient, position, margin, cellSize) {
        let align = 'center';
        let vAlign = 'middle';
        let x = point[0];
        let y = point[1];
    ...
    _renderWeekText(calendarModel, rangeData, orient, group) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const weekText = new graphic.Text({
                z2: 30,
                style: zrUtil.extend(createTextStyle(dayLabel, { text: nameMap[day] }), this._weekTextPositionControl(point, orient, pos, margin, cellSize))
            });
            group.add(weekText);
        }
    }
