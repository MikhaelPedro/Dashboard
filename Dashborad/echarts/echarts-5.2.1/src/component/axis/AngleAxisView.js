...
class AngleAxisView extends AxisView {
    ...
    render(angleAxisModel, ecModel) {
        ...
        const labels = zrUtil.map(angleAxis.getViewLabels(), function (labelItem) {
...
const angelAxisElementsBuilders = {
    ...
    axisLabel(group, angleAxisModel, polar, ticksAngles, minorTickAngles, radiusExtent, labels) {
        ...
        zrUtil.each(labels, function (labelItem, idx) {
            ...
            const cx = polar.cx;
            const cy = polar.cy;
            const labelTextAlign = Math.abs(p[0] - cx) / r < 0.3
                ? 'center' : (p[0] > cx ? 'left' : 'right');
            const labelTextVerticalAlign = Math.abs(p[1] - cy) / r < 0.3
