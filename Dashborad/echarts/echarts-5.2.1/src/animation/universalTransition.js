...
function transitionBetween(oldList, newList, api) {
    ...
    function createKeyGetter(isOld, onlyGetId) {
        return function (diffItem) {
            ...
            const dataGroupId = data.hostModel && data.hostModel.get('dataGroupId');
    ...
    (new DataDiffer(oldDiffItems, newDiffItems, createKeyGetter(true, useId), createKeyGetter(false, useId), null, 'multiple'))
        ...
        const oldElsList = filter(map(oldIndices, idx => oldDiffItems[idx].data.getItemGraphicEl(oldDiffItems[idx].dataIndex)), oldEl => oldEl && oldEl !== newEl && !isElementStillInChart[oldEl.id]);
        if (newEl) {
            stopAnimation(newEl);
        ...
        new DataDiffer(oldIndices, newIndices, (rawIdx) => oldDiffItems[rawIdx].data.getId(oldDiffItems[rawIdx].dataIndex), (rawIdx) => newDiffItems[rawIdx].data.getId(newDiffItems[rawIdx].dataIndex)).update((newIndex, oldIndex) => {
...
function getSeriesTransitionKey(series) {
    const seriesKey = series.getModel('universalTransition')
        .get('seriesKey');
    if (!seriesKey) {
